...
# Ex-vivo sensitivity to microenvironmental stimulation in primary CLL cells
```{r eval = FALSE, echo = FALSE}

#before snding
#tense
#author ship sectipn and starred authors 
#remove abstract text
#go thorugh and reference 
#go through and reference own paper 
#use pssive voice, and I , we if both invovled, go through and explain what we means 
#add contirbutions statement 
#reference edited figures if i need to 

#still to do on this chapter, after sending to Wolfgang 
#check the paligirms urles, make sure getting write 
#, update theme file as here, make package, then update lines to load data and remove funciotns 
#updata datfile so that has cyt label - remove from here 
#intro and methods dump 

#Next steps: 
#look thorough Gosia advice and cement plan
#Work out caching and knitting process, remove loading of clusters 
#set up git repo 
#Add function and data to package


#3 -4 more weeks progress 
```
...
#REMOVE ONCE I HAVE PACKAGE - MAKE SURE THEMES ARE UPDATED 
### ggplot themes
...
We selected 17 cytokines and microenvironmental stimuli, based on evidence in the literature that each stimulus had been shown to impact on CLL viability *in vitro*. We attempted to put together the largest panel of stimuli for a screen of this kind in CLL, aiming to minimise redundancy among the compounds (Bruch & Giles et al. 2021). 

CLL cells do not proliferate *in vitro*, but rather  undergo spontaneous apoptosis in the absence of stimulation [REFERENCE]. The effect of each stimuli on CLL viability was thus quantified by comparing ATP counts in treated primary samples, compared with those in DMSO wells where a positive value indicates that the sample viability was increased relative to control.
...
### *ex vivo* assay demonstrated functional diversity of cytokines and microenvironmental stimuli
To investigate heterogeneity amongst responses to the stimuli, I calculated Pearson correlation coefficients for each stimulus pair, using the log-transformed normalised viabilities. The resulting coefficients were ordered using hierarchical clustering and visualised in a symmetrical heatmap (Figure \@ref(fig:stimuliCorrelations)). The heirarchical clustering distinguished clusters of stimuli, including a larger group corresponding to agonists of TLR and Nf$\kappa$B pathways and a smaller group encompassing IL4 and TLR stimuli.

98.5% of stimulus pairs showed little correlation (R < 0.6), including those that targeted similar downstream pathways, indicating a high degree of functional diversity amongst microenvironmental signals. For example, JAK-STAT agonists such as IL4 and IL6 showed little correlation (Figure \@ref(fig:correlationsScatter)A). 

Only two stimulus pairs showed correlations R > 0.6, and in both cases these targeted near identical receptors or downstream pathways. These included CpG ODN (TLR 9) and Resiquimod (TLR 7 and 8) (Figure \@ref(fig:correlationsScatter)B), and IL4 and IL4 + CD40L which primarily target JAK3 - STAT6.

Repeating the analysis to correlate drug - drug pairs demonstrated that drugs targeting components of the same  pathway were highly correlated. For quality control purposes, this indicated that our data sensitively and specifically reflect inter-individual differences in pathway dependencies [REFERENCE](Dietrich et al. 2018).
 
(ref:stimuliCorrelations) Heatmap of Pearson correlation coefficients. Coefficients for each pair of stimuli were calculated using log transformed viability values normalised to untreated control, and ordered according to hierarchical clustering. 

(ref:correlationsScatter) Scatter plot of log-transformed viability values, normalised to DMSO controls,  for (A) treatment with JAK-STAT agonists IL4 and IL6 and (B) treatment with TLR agonists CpG ODN and Resiquimod. 

```{r stimuliCorrelations, fig.cap='(ref:stimuliCorrelations)', message = FALSE,  echo = FALSE, fig.height=5, fig.width = 5, fig.align="center", dev = 'cairo_pdf'}
...
```{r correlationsScatter, fig.cap='(ref:stimuliCorrelations)', message = FALSE,  echo = FALSE, fig.height=3, fig.width = 5, fig.align="center"}
...
tlr <-
  ...
  geom_point(colour=colors[1], size=3) + 
...
(ref:stimuliBeeswarms)  Log transformed viabilities after treatment with each stimulus. Where stimuli decreased viability relative to control, points are shown in blue, whilst increased viability is shown in red. 

```{r stimuliBeeswarms, fig.cap='(ref:stimuliBeeswarms)', fig.align="center", fig.width = 5, fig.height = 4,  echo = FALSE}
...
clusterStimuliTab %>% 
  ...
  ggplot(aes(x=Cytlabel, y=Log))+
  facet_grid(. ~ pathway, scales = "free",  space = "free") +
  geom_beeswarm(cex=0.4, alpha=0.7, size=2,
              aes(x = Cytlabel, y = Log, color =Log<0)) +
  scale_color_manual(values =c(palreds[7], palblues[2])) +
  guides(size="none", color = "none")+
  ylab("Log(Viability)") +
  xlab("")+
  t1 +
  theme(axis.text.x = element_text(angle = 45, vjust =1)) +
  theme(strip.background =element_rect(fill=colors[7]))+
...
```{r stimuliBeeswarmsVersion2,  fig.align="center", fig.width = 6, fig.height = 6,  echo = FALSE, eval = FALSE}
...
clusterStimuliTab %>% 
  ...
  geom_boxplot(width = 0.75, cex=0.4, alpha=0.7, size=2, aes(x =Cytlabel, y = Log,  fill =IGHV.status)) +
geom_beeswarm(cex=0.4, alpha=0.7, size=2,dodge.width = 0.75,
              aes(x =Cytlabel, y = Log, color = IGHV.status)) +
  scale_fill_manual(values =c(colors[6], colors[5])) +
  scale_color_manual(values =c(colors[6], colors[5])) +
  guides(size="none", color = "none")+
  ylab("Log(Viability)") +
  xlab("")+
  t1 +
...
### Microenvironmental response profiling identifies discrete patient subgroups
...
(ref:stimuliHeatmap) The heatmap matrix shows the viability measurements for 192 samples (columns) and 17 stimuli (rows). Viability was measured via ATP-based assay after 48h stimulation. The data are shown normalised to DMSO-treated controls, and scaled within each row according to the Median Absolute Deviation (MAD). Limits were applied to scaling factor for optimal visualisation. The colour bars to the right show sample annotations. Consensus Clustering was used to define column tree layout, using hierarchical clustering with the Euclidean metric. Figure from Bruch & Giles et al. 2021. 
...
#results = HGConsensusClusterPlus(d = viab.mat.lims, 
#                               maxK = 7, 
#                               reps = 10000, 
#                               pItem = 0.8, 
#                               pFeature = 1,  
...
```{r stimuliHeatmap, fig.cap='(ref:stimuliHeatmap)',  fig.width = 20, fig.height = 10, echo = FALSE}
...
pheatmap(viab.mat.lims,  
         ...
         fontsize=8,
         fontsize_row=12,
         legend_breaks = c(-3,0,3), width = 8, height = 8) 
...
I visualised the clustered heatmap in Figure \@ref(fig:stimuliHeatmap) for different values of k, and concluded on the existence of four robust clusters within the cohort (add these to appendix?). Each cluster shows a robust and unique response profile to the panel of stimuli. We termed the clusters C1 to C4: C1 and C2 were enriched for IGHV-U whilst the samples in C3 and C4 were mostly IGHV-M. 

To validate the choice of four clusters, I visualised the summaries of the consensus matrix, using the `ConsensusClusterPlus` package [REFERENCE], to quantify the degree of confidence in the clusters for different values of k (Figure \@ref(fig:consensusClustering)). 

(ref:consensusClustering) (A) Consensus CDF graphic showing the CDFs of the consensus matrix for k = 2 - 7, as indicated in the legend, estimated using 100 bin histogram. (B) Relative change in area under the CDF curve, for k = 2 - 7, to compare k with k - 1. In the case of k = 2, there is no k - 1, so the total area is plotted. Line shows relative increase in consensus between each value of k. (C) Assignment of patient samples (columns), to each cluster, for k = 1 - 7 (rows) to demonstrate stability of cluster membership. Cluster colour for k = 4 match those in heatmap in \@ref(fig:stimuliHeatmap). 

```{r consensusClustering, fig.cap='(ref:consensusClustering)', fig.align="center", echo = FALSE, eval = TRUE}
...
```
The graph of the CDFs of the consensus matrix for each k  indicated that the CDF reaches a maximum and cluster confidence is maximised at k = , and at k = , this is still very high. The graph showing relative change in the area under the CDF curve indicates the relative increase in consensus between different values of k. Between k = 4 to 5  there is a small appreciable increase, supporting the choice of k = 4. The cluster tracking plot depicts how each patient sample is assigned for each value of k. For k = 4, the plot indicates that clusters 3 and 4 are highly stable, whilst clusters 1 and 2 are less so.  



## Functional characterisation of patient clusters
###  C1 - C4 showed distinct response profiles with the panel of stimuli
...
Amongst the IGHV-U enriched  C1 and C2, both  showed strong, positive responses to IL4 and TLR7/8/9 stimulation.  C2 could be distinguished by stronger responses to the stimuli overall, in particular to NF$\kappa$B agonists IL1$\beta$,  anti-IgM, BAFF and sCD40L. Amongst the IGHV-M enriched clusters, C3 showed weaker responses to the majority of stimuli, and C4 was defined by a negative response to TLR7/8/9 stimulation. Figure \@ref(fig:clusterStimuliResponse) summarises these findings in  more detail, showing responses stratified by cluster for a subset of the stimuli.

(ref:clusterStimuliResponse) 


```{r clusterStimuliResponse, fig.cap='(ref:clusterStimuliResponse)', fig.height = 14, fig.width = 13, echo = FALSE}
...
df %>%
  ...
  dplyr::mutate(Cluster=factor(Cluster),
                ...
                pathway = factor(pathway,levels = c("TLR",   "NF\u03BAB","JAK-STAT","BCR", "TGF\u03B2")))
...
ggplot(clusterProfiling, aes(x=Cluster, y= Log,color = Cluster)) +
  ...
  facet_nested_wrap(. ~ pathway + Cytlabel, scales = "free", nrow = 3  ) +
  ylab("Log(Viability)")+
...
(ref:clusterLDTTTT) (A)  Lymphocyte doubling time (LDT) stratified by cluster, p-values from Student's t-test.  (B) Kaplan-Meier curves to show TTT for each cluster. p-values from univariate Cox proportional hazard models comparing IGHV-U enriched C1 with C2, and IGHV-M enriched C3 with C4. Figure from Bruch & Giles et al. 2021.
...
clusterLDT <-
  ...
  ylab("LDT in years") +
  t2 +
  stat_compare_means(method="t.test", comparisons = list(c(1,2), c(3,4)), size=6)
...
```{r clusterLDTTTT, fig.cap='(ref:clusterLDTTTT)',  echo = FALSE, fig.height = 6.5, fig.width = 11}
...
clusterTTT <-
  TTT_KM$plot +
  annotate(geom="text", x=6.6, y=0.5,    label=C3vsC4_p, color="black", size=6) +
  ...
  annotate(geom="text", x = 7.1, y = 0.23, label = C1vsC2_p, color="black", size=6) +
...
clusterLDT + clusterTTT +  plot_layout(design = thedesign) + plot_annotation(tag_levels = c('A'))
...
(ref:clusterCox) Table depicting results of multivariate Cox proportional hazard model to test prognostic value of key genetic features and clusters using TTT and C3 as reference. Table adapted from Bruch & Giles et al. 2021.

```{r clusterCox, tab.cap='(ref:clusterTTT)', echo = FALSE}
### TTT
...
Supp_table_4 %>%  
  kable(caption = "Table depicting results of Multivariate Cox Proportional Hazard Model to test prognostic value of key genetic features and clusters using Time to Next Treatment and C3 as reference. ") %>%
...
(ref:clusterDrugResponse) Log-transformed normalised viability values, stratified by cluster, for each drug. Drugs targeting the same pathway are grouped together. P-values from Student't t-test. 

```{r clusterDrugResponse, fig.cap='(ref:clusterDrugResponse)', fig.height = 13.5, fig.width = 13.5,fig.align="center", echo = FALSE}
...
df_complete %>%
  ...
  dplyr::mutate(Cluster=factor(Cluster),
                Drugpathway = factor(Drugpathway, levels = c("BCR", "mTOR", "DNA Damage Response","MAPK","NF\u03BAB", "JAK/STAT", "Epigenome", "HSP90"))) %>%
  ...
  guides(color="none")+
  #facet_nested(. ~ pathway + Cytokine, scales = "free" ) +
  facet_nested_wrap(. ~ Drugpathway + Drug, scales = "free", nrow = 3  ) +
  ylab("Log(Viability)")+
  coord_cartesian(clip="off") +
  stat_compare_means(method = "t.test", comparisons = list(c(1,2), c(3,4)))
...
### The clusters are enriched for different genetic features 
Next we assessed differences in the molecular profiles of samples within each cluster. We first investigated whether any of the clusters were enriched for genetic features common in CLL, by quantifying the enrichment and depletion of genetic features in each group (Bruch & Giles et al 2021).  

We ran a multinomial model, with lasso regularisation, to predict cluster membership (C1 -4) based on the matrix of genetic features for all the patient samples. The model assigned coefficients to genetic features, where a positive coefficient indicated that this feature was enriched in the cluster, and a negative coefficient indicated it was depleted. 

Our approach used 3-fold cross validation, selecting the optimal model using lamda min.  To ensure that the resulting coefficients were robust, we selected  coefficients that satisfied certain cut-offs. Coefficients were selected if they were assigned in > 60% of 50 bootstrapped repeats, and were larger than 0.35. Figure \@ref(fig:clusterEnrichment) shows the mean coefficients and associated standard deviation, for each genetic feature that met these criteria in each cluster. 

As we expected, IGHV status was the main feature that predicted cluster membership. Beyond IGHV status, Trisomy 12 and SF3B1 mutations were enriched in C2, which showed enhanced responses to many stimuli. C4, which was associated with slow in-vivo progression, showed depletion of TP53, ATM, RAS/RAF mutations and gain8q. 


(ref:clusterEnrichment) (A) Distribution of selected genetic features (rows) within each cluster for all patient samples (columns). Where a patient sample is not annotated for a feature, this is marked in white.  (B) Multinomial regression with lasso penalisation to identify enrichment or depletion of genetic features within each cluster. Matrix of genetic features  (p=39), and IGHV status (encoded as M = 1 and U = 0) were used to identify multivariate predictors of cluster assignment.  x axis shows genetic predictors, y axis indicates value and sign of coefficient assigned to feature, for each cluster (positive coefficients are enriched in the cluster, negative coefficients are depleted). Coefficients shown are mean coefficients from 50 bootstrapped repeats and  error bars represent the mean ± standard deviation.  Genetic features with >20% missing values were excluded, and only patients with complete annotation were included in the model (n=137). Figure from Bruch & Giles et al 2021. 
...
```{r clusterEnrichment, fig.cap='(ref:clusterEnrichment)',  fig.height = 6, fig.width = 13, warnings = FALSE,  echo  = FALSE}
...
geneticMatrix <- 
  ggplot(anno_long, aes(PatientID, feature)) +
      geom_tile(aes(fill = status), color = "grey",height=0.5, show.legend = FALSE) +
      #theme_classic() +
  t2+ 
...
geneticMatrix + clusterEnrichment + plot_annotation(tag_levels = c('A')) + plot_layout(widths = c(0.8, 1))


```


### GSEA of DE genes between subgroups
In addition to genetic features, we investigated differential expression of genes within each cluster. For n = 49 samples, we had RNAseq data available for matched PBMC samples. We focused on the difference between clusters 3 and 4, for which 21 RNAseq samples were available. 

To quantify differential gene expression, we began by filtering out immunoglobulin genes, including genes at the heavy, light and kappa loci that encode the antigen receptor of B cells. The clusters each show differential enrichment of IGHV-M and U samples, and thus the differential expression analysis would otherwise be dominated by immunoglobulin genes that are well known to affected by this biomarker. 

We followed the Deseq2 protocol using a design formula to quantify the difference between clusters, and accounting for the confounding effect of IGHV status.  ... genes were differentially expressed between C3 and 4 (Figure \@ref(fig:clusterDE)).


(ref:clusterDE)  Volcano plot of differentially expressed genes between Cluster 3 and Cluster 4. X axis indicates log2 fold change values, calculated using the Deseq package, y axis gives corresponding -log10(adjusted p value). P values adjusted using BH method. Genes are labeled where p value < 0.05.
...
```{r clusterDE, fig.cap='(ref:clusterDE)',  echo = FALSE}
...
To assign some biological meaning to the differentially expressed genes, we quantified the enrichment of Hallmark pathways amongst the genes. We ranked the genes based on the Wald statistic, as calculated by Deseq, and then ran GSEA was using the fgsea algorithm (Figure \@ref(fig:clusterGSEA)).

Several pathways were upregulated in C3, compared to C4, indicating that these pathways may relate in some way to the shorter TTT and LDT of samples within this cluster. Indeed pathways assosciated with higher disease aggression were upreagulated in C3 including pathways relating  stress response (Unfolded Protein Response, UV Response Up, P53 Pathway),  metabolism (Oxidative Phosphorylation) and proliferation (G2M Checkpoint, MYC Targets V1, MTORC1 Signaling, E2F Targets) (Figure \@ref(fig:GSEApathways)). 


(ref:clusterGSEA) Gene set enrichment analysis (GSEA) to compare expression of genes in samples from  C3 and C4 reveals upregulation of Hallmark pathways involved in microenvironmental signalling, stress response, metabolism and proliferation in C3. Normalised enrichment scores (NES) are shown for top 10 most significant pathways upregulated in C3 versus C4. Bars coloured according to adjusted p-value. Genes are ranked based on Wald statistic, calculated using the Deseq2 package and GSEA performed using the fgsea algorithm. 


```{r clusterGSEA, fig.cap='(ref:clusterGSEA)',  echo = FALSE}
...
In addition, C3 showed upregulation of  microenvironmental signalling pathways relative to C4, including TNFa Signalling via NFKB and Interferon Gamma Response (Figure \@ref(fig:GSEApathways)). This finding underlines our hypothesis that  differential activity of microenvironemtnal pathways, both *in vivo* and *ex vivo* may be relevant to disease prognosis. 

(ref:GSEApathways) Enrichment plots of selected pathways. Gene set enrichment analysis (GSEA) was performed with the Hallmark gene sets from the GSEA Molecular Signatures Database. Wald statistic was used to rank the genes.  The green curve corresponds to the Enrichment Score curve, which is the running sum of the weighted enrichment score obtained from GSEA software.

```{r GSEApathways, fig.cap='(ref:GSEApathways)',  echo = FALSE}
...
Can TLR support this (1 day work) 
...
<!--chapter:end:04-results-stimuli.Rmd-->
...
<!--chapter:end:05-results-stimuli-genes.Rmd-->
...
<!--chapter:end:06-results-drugs.Rmd-->
...
<!--chapter:end:07-results-ibrutinib-il4.Rmd-->
