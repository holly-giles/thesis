...
(ref:clusterLDTTTT) (A)  LDT stratified by cluster, p-values from Student's t-test.  (B) Kaplan-Meier curves to show TTT for each cluster. p-values from univariate Cox proportional hazard models comparing IGHV-U enriched C1 with C2, and IGHV-M enriched C3 with C4. See Methods Sections \@ref(LDT-method) and \@ref(survival-method). _Figure generated with Peter-Martin Bruch for the manuscript Bruch and Giles et al. 2021 ._
...
As expected, IGHV status was the main feature that predicted cluster membership. Beyond IGHV status, trisomy 12 and *SF3B1* mutations were enriched in C2, which showed enhanced responses to many stimuli. C4, which was associated with slow in-vivo progression, showed depletion of *TP53*, *ATM*, mutations in *RAS* and *RAF* and gain(8q) (Bruch and Giles et al. 2021). 
