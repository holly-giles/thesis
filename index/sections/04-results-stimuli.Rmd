...
### *ex vivo* assay demonstrated functional diversity of cytokines and microenvironmental stimuli {#cytokine-profiling}
...
(ref:stimuliCorrelations) Heatmap of Pearson correlation coefficients. Coefficients for each pair of stimuli were calculated using log transformed viability values normalised to untreated control, and ordered according to hierarchical clustering. See Methods section \@ref(correlations). _Figure adapted from Bruch & Giles et al. 2021._
...
(ref:consensusClusteringA) Summaries of the CDFs of the consensus matrices. Consensus CDF graphic showing the CDFs of the consensus matrix for k = 2 - 7, as indicated in the legend, estimated using 100 bin histogram (left). Relative change in area under the CDF curve, for k = 2 - 7, to compare k with k - 1. In the case of k = 2, there is no k - 1, so the total area is plotted. Line shows relative increase in consensus between each value of k (right). See Methods section \@ref(stimulus-heatmap-method).
...
(ref:clusterStimuliResponse) Log-transformed normalised viability values, stratified by cluster, for each stimulus. Stimuli activating the same pathway are grouped together. P-values from Student't t-test. _Figure adapted from (Bruch & Giles et al. 2021). _
...
(ref:clusterLDTTTT) (A)  Lymphocyte doubling time (LDT) stratified by cluster, p-values from Student's t-test.  (B) Kaplan-Meier curves to show TTT for each cluster. p-values from univariate Cox proportional hazard models comparing IGHV-U enriched C1 with C2, and IGHV-M enriched C3 with C4. See Methods sections \@ref(LDT-method) and \@ref(survival-method). _Figure from Bruch & Giles et al. 2021._
...
(ref:clusterCox) Table depicting results of multivariate Cox proportional hazard model to test prognostic value of key genetic features and clusters using TTT and C3 as reference. See Methods section \@ref(survival-method). _Table adapted from Bruch & Giles et al. 2021._
...
### GSEA of DE genes between subgroups {#cluster-rna}
...
(ref:clusterDE)  Volcano plot of differentially expressed genes between C3 and C4. X axis indicates log2 fold change values, calculated using the `DESeq2` package [@DESeq2], y axis gives corresponding -log10(adjusted p value). P values adjusted using BH method. Genes are labelled where adjusted p < 0.05. Methods section \@ref(cluster-rna-method)  _Figure from Bruch & Giles et al. 2021._
