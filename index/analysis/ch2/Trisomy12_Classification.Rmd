...
## ROC of best model
```{r, ROC,  fig.height=8, fig.width=10, fig.path=plotDir, dev=c("png", "pdf")}
...
## Average AUC
(if using test ration)
```{r, fig.width=4, fig.height=2, fig.path=plotDir, dev=c("png", "pdf")}

if(testRatio){
  
plotTab <- tibble(CV_set = glmRes$aucCV, 
                  Test_set = glmRes$aucTest) %>%
  gather(key = "type", value = auc) %>%
  group_by(type) %>% summarise(meanAuc = mean(auc),
                               sdAuc = sd(auc))

ggplot(plotTab, aes(x=type, y = meanAuc, fill = type)) + 
  geom_col(width = 0.4) +
  geom_errorbar(aes(ymin = meanAuc - sdAuc, ymax = meanAuc + sdAuc), width =0.2) +
  ylim(0,1) + xlab("") + ylab("mean AUC") +
...
#TIdy up 
Tri12 <- Tri12[,c(1,2, 4:23)]


#Identify patients that are misclassified more than 50% of cases
#filter dataframe for just WT patients
WT <- dplyr::filter(Tri12, trisomy12 == 0)

#Convert factors to numeric
WT[,3:22] <- sapply(WT[3:22], as.character)
WT[,3:22] <- sapply(WT[3:22], as.numeric)

#Make row names into column
WT <- WT%>%column_to_rownames("Row.names")

#COmpute row sums
WT$Total <- rowSums(WT)

#convert row names back to column
WT <- WT %>% rownames_to_column("PatientID")
WT %>% DT::datatable()

#Filter for misclassified patients 
Pats <- dplyr:: filter(WT, Total>9) %>% dplyr::select(PatientID)
print(Pats)
...
Any further profiling 

Hopefully good to here = have a predictor i can talk about 
get figures and conclusion out 

Then get a few figures to profile the misclassified patients 

-----
= additioanl figures are sorted

next: 
tidy up figures 

if time, text, otherwise three more days to finish chapter 5, then a week behind my schedule 

----

also this afternoon: 
30 mins - 1 hr to stay on top of other things 


# can also: 
check out the copy number varation for both of these patients
check out their RNA expression profiles 
whats their gene dosage for STAT6, IRAK4, SMAD3, SPIB, PU1? 


# SPIB and PU1 features of missclassified patients 
```{R }
genes <- c("ENSG00000269404", "ENSG00000066336")
